...
export function App() {
  ...
  async function complete40() {
    if (!analysis) return;
    setError(''); setLoading('Completing all 40 questions. Review will not interrupt this process…');
    try {
    ...
    <main className="app-shell">
      <aside className="sidebar">
        <div>
          ...
          <p>Local Teacher Studio · v0.2.0</p>
        </div>
        <nav>
      ...
      <section className="workspace">
        <header className="hero">
          <div><span className="eyebrow">SINGLE-USER LOCAL FACTORY</span><h2>Source → 40Q → Listening → Optional Review</h2><p>Google Form, ZIP, PDF, DOCX, XLSX/CSV, text, audio and video sources flow into one teacher production workspace.</p></div>
        ...
        {tab === 'source' && <SourceFactory url={url} setUrl={setUrl} files={files} setFiles={setFiles} analysis={analysis} onAnalyzeUrl={analyzeUrl} onAnalyzeFiles={analyzeFiles} onComplete={complete40} busy={!!loading} />}
        {tab === 'set' && <SetBuilder set40={set40} questions={setQuestions} reviewOpen={reviewOpen} setReviewOpen={setReviewOpen} selectedQuestion={selectedQuestion} selectedId={selectedId} setSelectedId={setSelectedId} onEdit={editQuestion} onRegenerate={regenerate} onAudio={regenerateAudio} onPublish={publishLocal} busy={!!loading} />}
...
function SourceFactory(props: { url: string; setUrl: (v: string) => void; files: File[]; setFiles: (v: File[]) => void; analysis: ImportAnalysis | null; onAnalyzeUrl: () => void; onAnalyzeFiles: () => void; onComplete: () => void; busy: boolean }) {
  const a = props.analysis;
  return <>
    <section className="panel"><div className="panel-heading"><div><span className="step">01</span><h3>Source Inbox</h3></div><span className="subtle">Form link or local files</span></div>
      <div className="stack-gap"><div className="url-row"><input value={props.url} onChange={e => props.setUrl(e.target.value)} placeholder="Paste Google Forms viewscore/result link" /><button onClick={props.onAnalyzeUrl} disabled={!props.url || props.busy}>Analyze Form</button></div>
      <div className="drop-row"><label className="file-picker">Choose ZIP / PDF / DOCX / XLSX / CSV / Audio / Video<input type="file" multiple onChange={e => props.setFiles(Array.from(e.target.files ?? []))} /></label><span>{props.files.length ? `${props.files.length} file(s) ready` : 'No files selected'}</span><button onClick={props.onAnalyzeFiles} disabled={!props.files.length || props.busy}>Analyze Files</button></div></div>
    </section>
    ...
    <section className="panel"><div className="panel-heading"><div><span className="step">02</span><h3>Analysis Result</h3></div><span className="subtle">{a.sourceTitle}</span></div><div className="analysis-toolbar"><span><strong>{a.questions.length}</strong> extracted questions</span><span><strong>{a.questions.filter(q => q.qaFlags.length).length}</strong> flags recorded (non-blocking)</span><button className="primary push-right" onClick={props.onComplete} disabled={props.busy}>Complete All 40 Questions</button></div><div className="question-grid compact">{a.questions.slice(0, 12).map(q => <QuestionMini key={q.id} question={q}/>)}</div>{a.questions.length > 12 && <p className="subtle center">+ {a.questions.length - 12} more imported questions</p>}</section></>}
  </>;
}

function SetBuilder(props: { set40: ExamSet | null; questions: NormalizedQuestion[]; reviewOpen: boolean; setReviewOpen: (v: boolean) => void; selectedQuestion: NormalizedQuestion | null; selectedId: string | null; setSelectedId: (v: string) => void; onEdit: (q: NormalizedQuestion, patch: Partial<NormalizedQuestion>) => void; onRegenerate: (q: NormalizedQuestion, mode: 'entire'|'choices'|'explanation'|'script') => void; onAudio: (q: NormalizedQuestion) => void; onPublish: () => void; busy: boolean }) {
  if (!props.set40) return <Empty title="No 40Q set yet" text="Import a source first, then press Complete All 40 Questions."/>;
  const set = props.set40;
  return <section className="panel"><div className="panel-heading"><div><span className="step">40</span><h3>{set.name ?? 'EPS 40Q Set'}</h3></div><span className={set.complete ? 'complete-badge' : 'pending-badge'}>{set.slots.filter(s => s.question).length} / 40</span></div>
    <p className="review-policy">Generation and QA finish all 40 first. You can use/save the completed set without reviewing. Open Optional Review only when you want to change a specific question.</p>
    ...
    <div className="slot-grid">{set.slots.map(slot => <button key={slot.slot} className={`slot ${slot.question ? 'filled' : 'missing'} ${props.selectedId === slot.question?.id ? 'selected' : ''}`} onClick={() => slot.question && props.setSelectedId(slot.question.id)}><strong>{slot.slot}</strong><span>{slot.patternId}</span><small>{slot.question?.type ?? 'missing'}</small><em>{slot.question?.qa?.score ?? '—'}</em></button>)}</div>
    {props.reviewOpen && props.selectedQuestion && <ReviewEditor question={props.selectedQuestion} onSave={patch => props.onEdit(props.selectedQuestion!, patch)} onRegenerate={mode => props.onRegenerate(props.selectedQuestion!, mode)} onAudio={() => props.onAudio(props.selectedQuestion!)} busy={props.busy}/>} 
  </section>;
}
...
function ReviewEditor({ question, onSave, onRegenerate, onAudio, busy }: { question: NormalizedQuestion; onSave: (patch: Partial<NormalizedQuestion>) => void; onRegenerate: (mode: 'entire'|'choices'|'explanation'|'script') => void; onAudio: () => void; busy: boolean }) {
  ...
  return <div className="review-editor"><div className="review-head"><div><strong>Q{question.sourceOrder} · {question.patternId}</strong><span>Chapter {question.chapter.chapter ?? '?'} · {question.type} · rev {question.revision ?? 1}</span></div><div className="qa-score">QA {question.qa?.score ?? '—'}</div></div>
    {question.audioAsset && <audio controls src={question.audioAsset.url}/>}<textarea value={stem} onChange={e => setStem(e.target.value)}/>{options.map((option, i) => <div className="option-edit" key={i}><input type="radio" checked={answer === i} onChange={() => setAnswer(i)}/><input value={option} onChange={e => { const next=[...options]; next[i]=e.target.value; setOptions(next); }}/></div>)}<textarea className="explain" value={explanation} onChange={e => setExplanation(e.target.value)} placeholder="Myanmar explanation"/>
...
function SettingsPanel({ status, profiles, onProfiles, onRefresh }: { status:SystemStatus|null; profiles:VoiceProfile[]; onProfiles:(v:VoiceProfile[])=>void; onRefresh:()=>void }) {
  ...
  const [draft, setDraft] = useState<VoiceProfile | null>(profile ?? null);
